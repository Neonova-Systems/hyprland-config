...
-- Tags
hl.window_rule({ match = { class = "^(firefox.*|librewolf|qutebrowser|chromium|google-chrome|zen-browser|Brave-browser|thorium|epiphany|ladybird|Mullvad%sBrowser|tor-browser)$" }, tag = "+browser" })
hl.window_rule({ match = { class = "^(code.*|VSCodium|jetbrain-.+|nvimport|nvim-qt|emacs.*|neovide|sublime_text|Helix|zed|cursor|lapce|eclipse|QtCreator)$" }, tag = "+code" })
hl.window_rule({ match = { class = "^(foot.*|kitty|Alacritty|wezterm|ghostty|rio|tabby|blackbox)$" }, tag = "+terminal" })
hl.window_rule({ match = { class = "^(steam|lutris|heroic|gamescope|com.heroicgameslauncher.hgl)$"}, tag = "+game"})
hl.window_rule({ match = { class = "^(retroarch|mGBA|Snes9x|PCSX2|dolphin-emu|yuzu|ryujinx|mgba.*|citra|rpcs3|duckstation|ppsspp|minecraft.*|PrismLauncher)$"}, tag = "+game-emulator"})
hl.window_rule({ match = { class = "^(mpv|vlc|celluloid|clapper|haruna|spotify|audacious|rhythmbox|amberol|io.github.CelluloidPlayer)$" }, tag = "+media" })
hl.window_rule({ match = { class = "^(obs|com.obsproject.Studio|Gimp.*|krita|Inkscape|blender|darktable|kdenlive|audacity|resolve|freecad)$" }, tag = "+creative-media" })
hl.window_rule({ match = { class = "^(discord|vesktop|Slack|TelegramDesktop|org.telegram.desktop|Element|signal|whatsapp.*|teams|zoom|matrix-.*|thunderbird|evolution)$" }, tag = "+communication" })
hl.window_rule({ match = { class = "^(thunar|nemo|pcmanfm.*|org.gnome.Nautilus|dolphin|yazi|ranger|doublecmd|krusader)$" }, tag = "+file-manager" })
hl.window_rule({ match = { class = "^(KeePassXC|Bitwarden|1Password|proton-pass|seahorse|gnome-passwords.*)$" }, tag = "+password-manager" })
